...
static const bool system_endian_runtime() {
	constexpr uint32_t x(0x03020100);
...
template <bool BITS_L2M>
std::size_t BitSequence<BITS_L2M>::bytes_covered_by_next_bits(
...
template <bool BITS_L2M>
void BitSequence<BITS_L2M>::fwd_skip_next(std::size_t bitcount) {
...
// Constructors
template <bool BITS_L2M>
BitSequence<BITS_L2M>::BitSequence() : p_index(nullptr), p_end(nullptr) {
	//assert(sizeof(unsigned long long) == 1
	//		|| endian::native != endian::other
	//	);
}

template <bool BITS_L2M>
BitSequence<BITS_L2M>::BitSequence(uint8_t* begin, uint8_t* end)
...
//template <bool BITS_L2M>
//BitSequence<BITS_L2M>::BitSequence(const BitSequence& rhs) :
//		subindex(rhs.subindex),
//		p_index(rhs.p_index),
//		p_end(rhs.p_end) {}
template <bool BITS_L2M>
void BitSequence<BITS_L2M>::init(uint8_t* begin, uint8_t* end) {
	set_fwd_subindex(0);
	p_index = BYTES_ASCENDING() ? begin : end-1;
	p_end = BYTES_ASCENDING() ? std::max(begin,end) : std::min(begin,end)-1;
}
...
template <bool BITS_L2M>
BitSequence<BITS_L2M>::operator bool() const {
	return has_next();
}
template <bool BITS_L2M>
std::size_t BitSequence<BITS_L2M>::bits_left() const {
	return CHAR_BIT*sizeof(uint8_t)*bytes_left() - fwd_subindex();
}
template <bool BITS_L2M>
bool BitSequence<BITS_L2M>::has_next() const {
	return bytes_left() > 0;
}
template <bool BITS_L2M>
bool BitSequence<BITS_L2M>::has_next(std::size_t bitcount) const 
{
	return bitcount <= bits_left();
}
template <bool BITS_L2M>
void BitSequence<BITS_L2M>::skip_next(std::size_t bitcount) {
	fwd_skip_next(std::min(bits_left(), bitcount));
}

template <bool BITS_L2M>
bool BitSequence<BITS_L2M>::peek_next() const {
	if (has_next()) {
...
template <bool BITS_L2M>
bool BitSequence<BITS_L2M>::read_next() {
	bool bit;
	if (has_next()) {
...
template <bool BITS_L2M>
void BitSequence<BITS_L2M>::write_next(bool bit) {
	if (has_next()) {
...
template <bool BITS_L2M>
inline BitSequence<BITS_L2M>& BitSequence<BITS_L2M>
		::operator>>(bool& bit)
{
	bit = read_next();
	return *this;
...
template <bool BITS_L2M>
template <typename MBYTE>
std::size_t BitSequence<BITS_L2M>::fwd_subindex_first_1bit(
		MBYTE value)
{
	return BITS_L2M
...
template <bool BITS_L2M>
template <typename MBYTE>
bool BitSequence<BITS_L2M>::read_streak_loopfunc(
...
std::size_t BitSequence<BITS_L2M>::read_streak(
		bool bit,
		std::size_t max_bitcount
	)
{
	static_assert(MAX_M > 0,
		"cannot operate with zero-length machine words");
	static_assert(MAX_M == 1 || endian::native != endian::other,
...
template <bool BITS_L2M>
template <typename MBYTE>
void BitSequence<BITS_L2M>::write_streak_loopfunc(
		bool bit, 
		std::size_t& bitcount
	)
{
...
template <bool BITS_L2M>
template <std::size_t MAX_M>
void BitSequence<BITS_L2M>::write_streak(bool bit, std::size_t bitcount) {
	static_assert(MAX_M > 0,
		"cannot operate with zero-length machine words");
	static_assert(MAX_M == 1 || endian::native != endian::other,
		"unsupported system endianness");
...
template <bool BITS_L2M>
template <typename MBYTE>
void BitSequence<BITS_L2M>::write_from_int(MBYTE source, std::size_t bitcount) {
	bitcount = std::min(bitcount, CHAR_BIT*sizeof(MBYTE));
	BitSequence<BITS_L2M> bseq2(
	...
	write_from<sizeof(MBYTE)>(bseq2, bitcount);
}

template <bool BITS_L2M>
template <typename MBYTE>
void BitSequence<BITS_L2M>::read_to_int(MBYTE& dest, std::size_t bitcount) {
	...
	bseq2.write_from<sizeof(MBYTE)>(*this, bitcount);
}
...
template <bool BITS_L2M>
template <typename MBYTE>
void BitSequence<BITS_L2M>::write_from_loopfunc(
		BitSequence& source,
		std::size_t& bitcount
	)
{
...
template <bool BITS_L2M>
template <std::size_t MAX_M>
void BitSequence<BITS_L2M>::write_from_innerfunc(
		BitSequence& source,
		std::size_t bitcount
	)
{
...
template <bool BITS_L2M>
template <std::size_t MAX_M>
void BitSequence<BITS_L2M>::write_from(BitSequence& source,std::size_t bitcount)
{
	static_assert(MAX_M > 0,
		"cannot operate with zero-length machine words");
	static_assert(MAX_M == 1 || endian::native != endian::other,
	...
	if (// source would override its end bytes when copying forward; i.e.,
		...
		typedef BitSequence<!BITS_L2M> BitSequence_REV;
...
std::ptrdiff_t operator-(
	...
	return CHAR_BIT * (BitSequence<BITS_L2M>::BYTES_ASCENDING()
